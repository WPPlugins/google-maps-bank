...
if (!is_user_logged_in()) {
    ...
    if (!$access_granted) {
        ...
        if (isset($google_maps_marker_data) && !empty($google_maps_marker_data)) {
            foreach ($google_maps_marker_data as $markers) {
                ?>
                        ...
                        {
                <?php
                $url = isset($markers["marker_icon_url"]) && $markers["marker_icon_url"] != "" ? esc_attr($markers["marker_icon_url"]) : GOOGLE_MAP_DEFAULT_MARKER_ICON;
                $check_image = google_maps_bank_get_icon_url($url);
                if ($check_image) {
                    ?>
                                icon = "<?php echo $url; ?>";
                    <?php
                    ...
                    ?>
                                icon = "<?php echo GOOGLE_MAP_DEFAULT_MARKER_ICON; ?>";
                    <?php
                }
                ?>
                        }
                        else if ("<?php echo esc_attr($markers["marker_icon_type"]) == "upload" ?>")
                        {
                            icon =
                                    {
                                        url: "<?php echo esc_attr($markers["marker_icon_upload"]); ?>",
                        ...
                        var marker = new google.maps.Marker
                                ({
                                    ...
                                    icon: icon,
                                    map: map,
                                    position: new google.maps.LatLng("<?php echo floatval($markers["marker_latitude"]); ?>", "<?php echo floatval($markers["marker_longitude"]); ?>")
                                });
                <?php
        ...
        ?>
            ...
            {
                var shortCutFunction = jQuery("#toastTypeGroup_error input:checked").val();
                var $toast = toastr[shortCutFunction](<?php echo json_encode($gm_confirm_choose_map) ?>);
            }
                                                ...
                                                if (layers_request == "")
                                                {
                                                    switch (overlay_request)
                                                    {
                                                        case "":
                                                            ...
                                                            break;
                                                        case "marker":
                                                            var marker_icons_src = jQuery("#ux_div_show_map_icons_ability .googlemaps-icons").find(".selected").attr("src");
                                                            ...
                                                            {
                                                                var shortCutFunction = jQuery("#toastTypeGroup_error input:checked").val();
                                                                var $toast = toastr[shortCutFunction]("<?php echo $gm_error_choose_icon_message; ?>");
                                                   }
                                                            else
                                                            {
                            ...
                            jQuery("#ux_frm_add_layers").validate
                                    ({
                                        submitHandler: function (form)
                                        {
                                            ...
                                            {
                                                var shortCutFunction = jQuery("#toastTypeGroup_error input:checked").val();
                                                var $toast = toastr[shortCutFunction]("<?php echo $gm_confirm_choose_map; ?>");
                                           }
                                            else
                                            {
                                                premium_edition_notification_google_maps_bank();
                ...
                case "gmb_shortcode":
                    ...
                    if (shortcodes_google_map == "1") {
                        ?>
                            ...
                            {
                                var shortCutFunction = jQuery("#manage_messages input:checked").val();
                                var $toast = toastr[shortCutFunction](<?php echo json_encode($gm_copy_to_clipboard) ?>);
                            });
                            clipboard.on("error", function (e)
                            {
                                var shortCutFunction = jQuery("#toastTypeGroup_error input:checked").val();
                                var $toast = toastr[shortCutFunction](<?php echo json_encode($gm_copied_failed) ?>);
                            });
